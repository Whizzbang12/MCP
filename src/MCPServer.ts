...
server.tool(
  ...
  "Fetches upcoming events and activities for ACM. " +
  ...
  async ({ category }) => {
    ...
    const filtered = category
      ? events.filter(e =>
          (e["Category"] ?? "").toLowerCase() === category.trim().toLowerCase()
        )
      : events;
...
server.tool(
  ...
  "Searches the ACM FAQ for general information about the organization. " +
